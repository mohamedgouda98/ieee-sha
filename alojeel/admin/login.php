...
<!-- Start PHP Code -->
  ...
  if(isset($_POST['submit'])){
    ...
    $select_admin = $cont->prepare("SELECT COUNT(id) FROM users WHERE email=? AND password=? LIMIT 1");
    $select_admin->execute(array($email , $pass));
    $admin = $select_admin->fetchColumn();

    if($admin ==1){
      $select_admin_data = $cont->prepare("SELECT * FROM users WHERE email=? AND password=?");
      $select_admin_data->execute(array($email , $pass));
      while($row = $select_admin_data->fetch()){
